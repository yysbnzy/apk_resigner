...
本工具用于 APK 签名完整性测试，适用于 GB 44495 / R155 等法规的 OTA 安全测试。
...
🔧 修改内容+签名
  • 自动检测原 APK 的签名方案（V1/V2/V3/V4）
  • 直接解压 APK（无需反编译）
  • 在 assets/ 目录下添加 test.txt 标记文件
  • 重新 zip 打包 APK
  • zipalign 对齐
  • 按原签名方案重新签名（新密钥）
  • 输出：内容被修改 + 签名被替换的 APK
...
|------|------|----------|
| 正常安装 | 原始 APK | 系统允许安装 |
| 内容+签名篡改 | 🔧 修改内容+签名 | 系统拒绝安装 |

═══════════════════════════════════════
【注意事项】
═══════════════════════════════════════

• 签名后的 APK 无法通过原签名校验，系统安装时会失败
• 这是预期行为，用于测试系统的完整性校验机制
• 工具会自动生成测试密钥，也可使用自定义密钥库
